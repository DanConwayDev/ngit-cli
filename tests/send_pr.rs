...
/// exit. Mirrors the `run_git` helper in `tests/send_grasp.rs`.
...
/// avoid pulling the git2 library into the test's critical path; the bare
/// repo is already on-disk, so a subprocess call is cheap.
///
/// Returns an error if the ref does not exist (missing ref = the push never
/// landed, a genuine test failure).
...
/// Matches the same helper in `tests/init_state_fresh.rs`.
...
/// Used for multi-value tags (`clone`, `relays`).
/// Matches the same helper in `tests/init_state_fresh.rs`.
