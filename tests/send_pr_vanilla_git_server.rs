...
//! 5. **Maintainer advances `main`** so `merge_base_oid ≠ main_tip_at_send_time`.
...
async fn capture_snapshot() -> Result<Snapshot> {
    ...
    let grasp_primary_pr_ref_oid = read_nostr_ref(&bare_primary, &pr_event_id_hex)
        ...
        .with_context(|| {
            format!(
                "reading refs/nostr/{pr_event_id_hex} from primary grasp bare repo at {}",
    ...
    let vanilla_bare = harness.vanilla_git_server("extra").repo_path().to_path_buf();
...
/// Run `git <args>` inside `repo`, bailing with captured output on failure.
...
/// repository at `bare_repo`, using `git for-each-ref`.
///
/// Returns an error if the ref does not exist — a missing ref means the push
/// never landed (genuine test failure, not a configuration issue).
