...
- Store `nostr.nsec` and `nostr.bunker-app-key` secrets in the OS credential store, leaving only pointer entry names in git config; plaintext values migrate automatically (with a one-time notice) and remain supported where no credential store is available. Set `nostr.credential-store` to `false` or `NGIT_CREDENTIAL_STORE=false` to opt out. See `docs/credential-storage.md`.
