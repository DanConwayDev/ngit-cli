...
- Store `nostr.nsec` and `nostr.bunker-app-key` secrets in the OS credential store, leaving only pointer entry names in git config; plaintext values migrate automatically and remain supported where no credential store is available.
