...
- GRASP service URLs may include a non-root base path. Repository announcements, `nostr://` relay hints, Git push/fetch, and explicit GRASP-06 PR endpoints preserve the configured path; the integration harness now exercises those flows against a path-mounted ngit-grasp subprocess.
