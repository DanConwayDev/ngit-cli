...
- **Top-level `command_status` reports the ngit command outcome.** `ok` corresponds to exit code 0 and `error` to a non-zero exit. It does not describe a nested entity or domain result; for example, `ngit ci status --json` can report `command_status: "ok"` with `ci.conclusion: "failure"` when no CI gate was requested.
