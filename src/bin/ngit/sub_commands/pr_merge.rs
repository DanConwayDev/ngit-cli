//! Canonical implementation for `ngit pr merge` and its top-level alias.
...
use ngit::{
    ...
    git_events::{
        ...
        get_commit_id_from_patch, get_pr_tip_event_or_most_recent_patch_with_ancestors, get_status,
        is_event_proposal_root_for_branch, pr_event_clone_tag_urls, process_cover_note,
        process_subject, status_kinds, tag_value,
    },
...
use nostr::prelude::{EventId, Kind, PublicKey, RelayUrl, ToBech32, nip19::Nip19Event};
...
pub async fn launch(
    ...
    // Explicit IDs can resolve closed and already-applied proposals, while
    // branch inference deliberately considers only open or draft proposals.
    // Enforce the lifecycle guard after either resolution path and before CI
    // evaluation or any Git mutation.
    let statuses = {
        let mut statuses = get_events_from_local_cache(
            ...
            vec![
                nostr::prelude::Filter::default()
                    .kinds(status_kinds())
                    .events(proposals_and_revisions.iter().map(|event| event.id)),
                nostr::prelude::Filter::default()
                    .custom_tags(
                        nostr::filter::SingleLetterTag::UPPERCASE_E,
                        proposals_and_revisions.iter().map(|event| event.id),
                    )
                    .kinds(status_kinds()),
            ],
        )
        .await?;
        statuses.sort_by_key(|event| event.created_at);
        statuses.reverse();
        statuses
    };
    let proposal_root_events = proposal_roots(&proposals_and_revisions)
        .cloned()
        .collect::<Vec<_>>();
    match get_status(&proposal, &repo_ref, &statuses, &proposal_root_events) {
        Kind::GitStatusApplied => bail!("PR is already applied/merged"),
        Kind::GitStatusClosed => bail!("PR is closed; reopen it before merging"),
        _ => {}
    }
...
/// The JSON document emitted for a local merge outcome or a CI refusal.
///
/// Every successful command retains the canonical PR identity and outcome,
/// including a conflicted merge handed back for manual resolution. CI fields
/// are present only when the caller requested a gate, and there is no event
/// field because publishing the applied status is deferred to the later Git
/// push.
fn merge_json(
    proposal_id: EventId,
    ...
    if let Some(ci) = ci {
        document["ci"] = ci.to_ci_value(relay, LogTailMode::Auto);
        document["ci_warning"] = Value::Null;
    }
...
fn ensure_target_available_in_current_worktree(git_repo: &Repo, target_branch: &str) -> Result<()> {
    ...
    for entry in worktrees.split("\n\n") {
        ...
        bail!(
            "target branch '{target_branch}' is checked out in another worktree at '{path}'. Run `ngit pr merge` from that worktree, or check out a different branch there first."
        );
    }
...
impl SavedWorktree {
    fn capture(git_repo: &Repo) -> Result<Option<Self>> {
        ...
        let message = format!("ngit pr merge automatic backup ({})", std::process::id());
...
async fn resolve_event_id_from_current_branch(
    ...
    if !branch.starts_with("pr/") {
        bail!("not on a `pr/` branch; specify a PR event-id or nevent, e.g. `ngit pr merge <id>`");
    }
...
/// published PR tip.
///
/// `ngit pr merge` autogenerates the merge commit message — subject, nevent,
